...
\title{\textbf{Ejercicio 1: Backpropagation (justificado paso a paso)}}
...
Se considera una red neuronal \textbf{feedforward} con \textbf{una neurona por capa} (caso escalar). La arquitectura es:
\[
...
\]
Función de pérdida para \textbf{un único ejemplo}:
\[
L\,=\,(\hat{y}-y)^2.
\]
\textbf{Nota:} al trabajar con un solo ejemplo, ``loss'' y ``cost'' coinciden numéricamente.
...
\]
\textbf{Importante:} los gradientes se calculan con los parámetros del forward actual (antes de aplicar la actualización).
